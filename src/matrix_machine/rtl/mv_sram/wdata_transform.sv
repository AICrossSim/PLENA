...
logic [SubTile_Index_Width-1:0] subtile_row_index, subtile_col_index, subtile_index;
logic [SubSRAM_Index_Width-1:0] start_subsram_index, subsram_index;
assign start_subsram_index = addr[Parallel_Rd_Index_Width-1:0]


always @(posedge clk) begin
    for (subtile_index = 0; subtile_index < SubSRAM_Amount * Parallel_Wr_Element_Amount; subtile_index++) begin
        
        subtile_row_index = subtile_index % Parallel_Rd_Dim;
        subtile_col_index = subtile_index / Parallel_Rd_Dim;
        subsram_index = (subtile_index + start_subsram_index) % SubSRAM_Amount

        for (row_index_in_tile = 0; row_index_in_tile < Parallel_Rd_Dim; row_index_in_tile++) begin
            sub_sram_wdata[subsram_index][ subtile_row_index * ElementWidth + (row_index_in_tile + 1) * ElementRowWidth - 1 +: ElementRowWidth] 
            <= in_data[  (subtile_row_index + row_index_in_tile) * MLEN * DataWidth + (subtile_col_index + 1 ) * ElementRowWidth - 1 +: ElementRowWidth];
        end
    end
end
