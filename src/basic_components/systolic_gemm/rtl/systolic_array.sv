...
module systolic_array #(
    ...
    logic rowwise_data_transfer_valid [COMPUTE_DIM - 1:0][COMPUTE_DIM - 1:0];
    logic rowwise_data_transfer_ready [COMPUTE_DIM - 1:0][COMPUTE_DIM - 1:0];
    logic [MXFP_MANT_WIDTH + MXFP_EXP_WIDTH : 0]    rowwise_data_transfer_element   [COMPUTE_DIM - 1:0][COMPUTE_DIM - 1:0];
    logic [MXFP_SCALE_WIDTH - 1 : 0]                rowwise_data_transfer_scale     [COMPUTE_DIM - 1:0][COMPUTE_DIM - 1:0];

    logic columnwise_data_transfer_valid [COMPUTE_DIM - 1:0][COMPUTE_DIM - 1:0];
    logic columnwise_data_transfer_ready [COMPUTE_DIM - 1:0][COMPUTE_DIM - 1:0];
    logic [MXFP_MANT_WIDTH + MXFP_EXP_WIDTH : 0]    columnwise_data_transfer_element [COMPUTE_DIM - 1:0][COMPUTE_DIM - 1:0];
    logic [MXFP_SCALE_WIDTH - 1 : 0]                columnwise_data_transfer_scale [COMPUTE_DIM - 1:0][COMPUTE_DIM - 1:0];
    ...
    generate;
        for (genvar i = 0; i < COMPUTE_DIM; i = i + 1) begin : fill_with_input_data
            // Fill the Top Row
            assign rowwise_data_transfer_element[0][i] = in_top_element[i];
            assign rowwise_data_transfer_scale  [0][i] = in_top_scale[i / BLOCK_DIM];
            assign rowwise_data_transfer_valid[0][i] = distributed_in_top_valid[i];
            assign distributed_in_top_ready[i] = rowwise_data_transfer_ready[0][i];

            // Fill the Left Column
            assign columnwise_data_transfer_element[i][0] = in_left_element[i];
            assign columnwise_data_transfer_scale  [i][0] = in_left_scale[i / BLOCK_DIM];
            assign columnwise_data_transfer_valid[i][0] = distributed_in_left_valid[i];
            assign distributed_in_left_ready[i] = columnwise_data_transfer_ready[i][0];
        end
    endgenerate
    ...
    generate;
        for (genvar i = 0; i < COMPUTE_DIM; i = i + 1) begin : fill_extraction_signals
            assign rowwise_data_transfer_ready[i][COMPUTE_DIM - 1] = 1'b1;
            assign columnwise_data_transfer_ready[COMPUTE_DIM - 1][i] = 1'b1;
        end
    endgenerate
    ...
    // Computation
    generate;
        for (genvar i = 0; i < COMPUTE_DIM; i = i + BLOCK_DIM) begin : pe_row
            for (genvar j = 0; j < COMPUTE_DIM; j = j + BLOCK_DIM) begin : pe_col
                if (i == 0) begin
                    first_row_pe #(
