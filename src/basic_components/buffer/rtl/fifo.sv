  ...
  generate
    if (DEPTH == 1) begin : gen_register_slice
      ...
      always_comb begin
        ...
        // Input side ready
        data_in_ready = self.size != DEPTH;

        // Pause reading when there is (no transfer on this cycle) AND the registers are full.
        pause_reads = !data_out_ready && (self.out_reg.valid || self.extra_reg.valid);
        ...
        if (self.ram_dout_valid) begin
          if (self.out_reg.valid && !data_out_ready) begin
        ...
        if (self.next_reg) begin
          data_out  = self.extra_reg.data;
          data_out_valid = self.extra_reg.valid;
          if (data_out_ready && self.extra_reg.valid) begin
            next_self.extra_reg.valid = 0;
            next_self.next_reg = 0;
          end
        end else begin
          data_out  = self.out_reg.data;
          data_out_valid = self.out_reg.valid;
          if (data_out_ready && self.out_reg.valid) begin
