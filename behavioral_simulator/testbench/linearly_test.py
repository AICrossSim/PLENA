...
if __name__ == "__main__":
    ...
    fp_preload = [0.0, 1e-6, 1/hidden_size_in]

    # Gen Weight and Test Data
    # generate_and_save_random_weights(hidden_size, hidden_size, get_weights_path('model_weights.pt'))
    
    torch.manual_seed(42)
    act_tensor = torch.randn(batch_size, hidden_size_in)
    original_layer = nn.Linear(in_features=hidden_size_in, out_features=hidden_size_out, bias=False)
    ...
    # 权重矩阵大小是 hidden_size_in * hidden_size_out = 128 * 256 = 32768
    # activation 大小是 batch_size * hidden_size_in = 4 * 128 = 512

    gen_assembly_code += preload_addr_reg_asm(
        addr_reg_to_set=[1, 2],
        available_registers=[1, 2],
        addr_reg_val=[
            int(hidden_size_in * batch_size * real_data_ratio),  # activation 结束位置，权重开始位置
            int((hidden_size_in * batch_size + hidden_size_in * hidden_size_out) * real_data_ratio)  # 权重结束位置
        ]
    )
    ...
    gen_assembly_code += projection_asm(
        ...
        blen=4,
        batch=4,
        hidden_size_in=128,   # 输入维度
        hidden_size_out=256,  # 输出维度 (intermediate_size)
        alive_registers=[1,2,3,4],
        w_base_hbm_offset_reg=1,
        activation_base_address=0,
        result_base_address=0,  # 写到地址 0
        rope_enabled=False
    )
    
    with open("behavioral_simulator/testbench/linear_test.asm", "w") as asm_file:
        asm_file.write(gen_assembly_code)

    create_sim_env(input_tensor, gen_assembly_code, golden_result, fp_preload)
    create_mem_for_sim(data_size=1024, mode="behave_sim", asm="linear", data=None, specified_data_order = ["act_tensor", "weights"])
