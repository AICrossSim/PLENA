...
from tools.quant import get_quantized_cls, get_quantized_func
from .configuration_llama_quantized import LlamaQuantizedConfig
...
class LlamaQuantizedAttention(nn.Module):
    ...
    def __init__(self, config: LlamaQuantizedConfig, layer_idx: int):
        super().__init__()
        self.config = config
        self.layer_idx = layer_idx
        self.hidden_size = config.hidden_size
        ...
        self.k_proj = get_quantized_cls("linear", self.quant_config["k_proj"])(
            self.hidden_size, self.num_attention_heads * self.head_dim, bias=config.attention_bias, config=self.quant_config["k_proj"])
        self.v_proj = get_quantized_cls("linear", self.quant_config["v_proj"])(
            self.hidden_size, self.num_attention_heads * self.head_dim, bias=config.attention_bias, config=self.quant_config["v_proj"])
        self.o_proj = get_quantized_cls("linear", self.quant_config["o_proj"])(
            self.num_attention_heads * self.head_dim, self.hidden_size, bias=config.attention_bias, config=self.quant_config["o_proj"])
...
class LlamaQuantizedForCausalLM(LlamaQuantizedPreTrainedModel, GenerationMixin):
...
__all__ = ["LlamaQuantizedForCausalLM"]