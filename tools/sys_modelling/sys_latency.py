...
class sys_latency_config:
    def __init__(self, model_param_path, hardware_config, batch_size = 1, seq_len = 2048, output_token = 128, device_num = 1):
        ...
        self.vocab_size = model_param["vocab_size"]
        self.DataTypeSize = 1                               # 1 byte per element
    ...
    def projection(self, mode = "prefill"):
        if mode == "prefill":
            overall_inst_num = 0
            # Q, K Projection + RoPE
            overall_inst_num += self.batch_size * (math.ceil(self.hidden_size / self.hardware_config["BLEN"]) * (math.ceil(self.hidden_size / self.hardware_config["MLEN"]) * (math.ceil(self.input_seq_len / self.hardware_config["BLEN"]) * 2 + 10)))
            overall_inst_num += self.batch_size * (self.num_attention_heads * (self.input_seq_len // self.hardware_config["VLEN"])) * 3

            overall_inst_num += self.batch_size * (math.ceil((self.num_key_value_heads * self.head_dim) / self.hardware_config["BLEN"]) * (math.ceil(self.hidden_size / self.hardware_config["MLEN"]) * (math.ceil(self.input_seq_len / self.hardware_config["BLEN"]) * 2 + 10)))
            overall_inst_num += self.batch_size * (self.num_key_value_heads * (self.input_seq_len // self.hardware_config["VLEN"])) * 3
    ...
    def flash_attention(self, mode = "prefill"):
        ...
        assert self.num_key_value_heads <= (self.hardware_config["MLEN"] // self.hardware_config["BLEN"]) , "num_key_value_heads must be less than or equal to (MLEN // BLEN)"
        if mode == "prefill":
            # Outer loop
            for i in range(self.input_seq_len // self.hardware_config["MLEN"]):
                for j in range((self.input_seq_len // self.hardware_config["MLEN"])):
                    overall_inst_num += 4 + (mlen // blen) * 8 + 3 # MLEN * MLEN
                    overall_inst_num += 2 + mlen * 5  # Softmax
                    overall_inst_num += (9 + 4 + (math.ceil(tile_in_atten / blen)) * 8 + 3) #PV
                    overall_inst_num += (1+ tile_in_atten * 3) #Compute O
                    overall_inst_num += 8
                    # overall_inst_num += 2 + mlen * 4
        elif mode == "decode":
            for j in range(math.ceil(self.kv_size / self.hardware_config["MLEN"]) ):
                overall_inst_num += (4 + (tile_in_atten // blen) * 4 + 3)
                overall_inst_num += 2 + mlen * 5  # Softmax
    ...
    def residual (self, mode = "prefill"):
        ...
        if mode == "prefill":
            ...
            overall_inst_num = (5 * iteration + 3) * self.input_seq_len
        elif mode == "decode":
            iteration = self.hidden_size // self.hardware_config["VLEN"]
    ...
    def feed_forward(self, mode = "prefill"):
        ...
        # -- MLP
        if mode == "prefill":
            overall_inst_num += 2 * math.ceil(self.intermediate_size / blen) * math.ceil(self.hidden_size / mlen) * 4 * (self.input_seq_len // blen)
            overall_inst_num += math.ceil(self.intermediate_size / vlen) * 5
            overall_inst_num += math.ceil(self.intermediate_size / blen) * math.ceil(self.hidden_size / mlen) * 4 * (self.input_seq_len // blen)
    ...
    def embeddings(self, mode = "prefill"):
        ...
        overall_inst_num = 3
        if mode == "prefill":
            overall_inst_num += self.input_seq_len * math.ceil(self.hidden_size / blen) * math.ceil(self.hidden_size / mlen) * (blen * 2 + 1) + 4
        elif mode == "decode":
