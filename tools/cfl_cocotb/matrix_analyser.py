...
class packed_array_analyser:
    ...
    def print_write_data_to_sram(self, input_data, sram_to_monitor):
        ...
        for sram_index in sram_to_monitor:
            sub_sram_index = self.SRAM_Amount -1 - sram_index
            wdata_per_sram = binary_string_to_hex(input_data[sub_sram_index * wdata_width_per_sram: (sub_sram_index + 1) * wdata_width_per_sram], self.Data_Width)
    ...
    def print_read_data_from_sram(self, output_data, sram_to_monitor):
        ...
        for sram_index in sram_to_monitor:
            sub_sram_index = self.SRAM_Amount - 1 - sram_index
            rdata_per_sram = binary_string_to_hex(output_data[sub_sram_index * self.ElementWidth : (sub_sram_index + 1) * self.ElementWidth], self.Data_Width)
