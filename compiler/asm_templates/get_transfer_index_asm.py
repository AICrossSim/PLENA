...
def get_transfer_index_edge(
    ...
    generated_code += f"S_ADDI_INT gp{len_reg}, gp0, {hidden_size} \n"
    ...
    for t in range(T):  
        ...
        for batch_idx in range(batch_size):  
            for i in range(hidden_size):
                ...
                generated_code += f"S_ST_INT gp{max_idx_addr}, gp0, {(prompt_batch_size+0)*hidden_size + batch_idx*hidden_size + i} \n"
            
            # grab these accumulated hidden_size scalar(stored reciprocal) into SRAM to form an vector again
            # SRAM[gp<x0_p_addr>] = FP_MEM[gp0+0 : gp0+0+hidden_size]
            ...
            generated_code += f"S_ADDI_INT gp{x_addr}, gp0, {(prompt_batch_size+0)*hidden_size + batch_idx*hidden_size} \n"  # output base (hidden_size elements)
            generated_code += f"S_ADDI_INT gp{x0_addr}, gp0, {batch_idx*hidden_size} \n"        # x base (hidden_size elements, src2)
            # mask is in VECTOR SRAM at gp{mask_addr} (hidden_size float elements, already set up earlier)
            
            # Execute S_SELECT_INT: processes hidden_size elements in one instruction
...
def get_transfer_index_performance(
    ...
    for t in range(T):  
        ...
        for batch_idx in range(batch_size//batch_R):  
            ...
            hbm_offset  = t*batch_size*hidden_size*vocal_size + batch_idx*batch_R*hidden_size*vocal_size
            vram_offset = logits_base_address
            generated_code += preload_act_asm_scale(
                vlen=vlen,
                preload_len=1,
                batch=1,
                hidden_size=hidden_size*vocal_size*batch_R,
                scale=T*batch_size*hidden_size*vocal_size,
                alive_registers=[1,2,3],  # [a_actual_register, set_stride_register, result_register]
                act_hbm_offset=hbm_offset,
                act_vram_offset=vram_offset,
                activation_offset_reg=0
            ...
            for batch_R_idx in range(batch_R):
                for i in range(hidden_size):
                    logits_i_address = logits_base_address + (batch_R_idx*hidden_size + i) *vlen*(vocal_size // vlen)
                    ...
                    generated_code += f"S_ST_FP f2, gp0, {i} \n"
                    # Store the max_idx from gp{max_idx_addr} into INT_MEM, INT_MEM[int_reg<gp0>+ (prompt_batch_size+1)*hidden_size +(batch_idx*vlen + i)] = gp{max_idx_addr}
                    generated_code += f"S_ST_INT gp{max_idx_addr}, gp0, {(prompt_batch_size+0)*hidden_size + (batch_idx*batch_R+batch_R_idx)*hidden_size + i} \n"
                ...
                # Setup base addresses in registers for current batch
                generated_code += f"S_ADDI_INT gp{x_addr}, gp0, {(prompt_batch_size+0)*hidden_size + (batch_idx*batch_R+batch_R_idx)*hidden_size} \n"  # output base (hidden_size elements)
                generated_code += f"S_ADDI_INT gp{x0_addr}, gp0, {(batch_idx*batch_R+batch_R_idx)*hidden_size} \n"        # x base (hidden_size elements, src2)
                # mask is in VECTOR SRAM at gp{mask_addr} (hidden_size float elements, already set up earlier)
    ...
    generated_code += f"S_ADDI_INT gp{x0_p_addr}, gp0, {x0_p_base_address} \n"
    generated_code += f"S_ADDI_INT gp{len_reg}, gp0, {vlen} \n"
    ...
    for batch_idx in range(batch_size):  
        ...
        # Execute S_SELECT_INT: processes vlen elements in one instruction
        # x0 = torch.where(mask_index, x0, x)
        generated_code += f"S_SELECT_INT gp{x_addr}, gp{x_addr}, gp{x0_addr}, gp{mask_addr}, gp{len_reg} \n"
        # x[transfer_index] = x0[transfer_index]
        # equal to: x = torch.where(transfer_index, x0, x)
